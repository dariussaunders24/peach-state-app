...
function RouteHub({ event }: any) {
  ...
  const hasOldRouteLink = !!event.route_link;

  if (routes.length === 0 && !hasOldRouteLink) {
    return (
    ...
    <div className="mt-4 rounded-lg border border-[#F28C52]/20 bg-black/30 p-4">
      ...
      {hasOldRouteLink && (
        <div className="mt-4 rounded-lg border border-white/10 bg-black/20 p-3">
          <p className="font-semibold text-white">Primary Route / Map</p>

          <a
            href={event.route_link}
            target="_blank"
            rel="noopener noreferrer"
            className="mt-3 inline-block rounded-lg border border-[#F28C52] px-3 py-2 text-sm font-semibold text-[#F28C52] hover:bg-[#F28C52] hover:text-black"
          >
            Open Route / Map
          </a>
        </div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="mt-1 text-sm leading-6 text-gray-400">
                  Exact meetup location, Route Hub links, and event instructions are visible after RSVP.
                </p>
              </div>
            </div>
          )}
